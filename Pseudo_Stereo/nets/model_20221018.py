...
if __name__ == '__main__':
    ...
    cost_volume_net = CostVolume(max_disp=48, feature_similarity='correlation').to(device)
    left = torch.randn([1, 16, 32, 32]).to(device)
    right = torch.randn([1, 16, 32, 32]).to(device)
    x = {
        'left': left,
        'right': right
    }
    left_feats, right_feats = cost_volume_net(x)