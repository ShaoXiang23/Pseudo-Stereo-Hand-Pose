...
class ManoLayer_PCA(Module):
    ...
    def __init__(self,
                 ...
                 ncomps=6,
                 side='right',
                 mano_root='/home/robot/gsx/HandFit/utils/manopth/mano/models',
                 use_pca=True,
                 root_rot_mode='axisang',
                 joint_rot_mode='axisang',
                 robust_rot=False):
        ...
        Args:
            ...
            joint_rot_mode: 'axisang' or 'rotmat', ignored if use_pca
        """
        super(ManoLayer_PCA, self).__init__()
        ...
        self.register_buffer('th_betas',
                             torch.Tensor(smpl_data['betas'].r).unsqueeze(0).to(device))
        self.register_buffer('th_shapedirs',
                             torch.Tensor(smpl_data['shapedirs'].r).to(device))
        self.register_buffer('th_posedirs',
                             torch.Tensor(smpl_data['posedirs'].r).to(device))
        self.register_buffer(
            'th_v_template',
            torch.Tensor(smpl_data['v_template'].r).unsqueeze(0).to(device))
        self.register_buffer(
            'th_J_regressor',
            torch.Tensor(np.array(smpl_data['J_regressor'].toarray())).to(device))
        self.register_buffer('th_weights',
                             torch.Tensor(smpl_data['weights'].r).to(device))
        self.register_buffer('th_faces',
                             torch.Tensor(smpl_data['f'].astype(np.int32)).long().to(device))

        self.face = torch.Tensor(smpl_data['f'].astype(np.int32)).long()
        ...
        if self.use_pca or self.joint_rot_mode == 'axisang':
            # Save as axis-angle
            self.register_buffer('th_hands_mean', th_hands_mean.cuda())
            selected_components = hands_components[:ncomps]
            self.register_buffer('th_selected_comps',
                                 torch.Tensor(selected_components).cuda())
        else:
            th_hands_mean_rotmat = rodrigues_layer.batch_rodrigues(
                th_hands_mean.view(15, 3)).reshape(15, 3, 3)
            self.register_buffer('th_hands_mean_rotmat', th_hands_mean_rotmat.cuda())
    ...
    def pose_pca2Rotmat(self, th_pose_coeffs):
        ...
        th_full_pose = torch.cat([th_pose_coeffs[:, :self.rot].cuda(), self.th_hands_mean.cuda() + th_full_hand_pose],
                                 1)
        th_full_pose = th_full_pose.view(batch_size, -1, 3)
    ...
    def forward(self,
        ...
        # return th_verts, th_jtr,
        return th_verts, th_jtr, self.face.cuda()