...
def generate_coord(points, refpoint, new_size, angle, trans, sizes):
    cubic_size, cropped_size, original_size = sizes # [200, 88, 96]
...
def generate_cubic_input(points, refpoint, new_size, angle, trans, sizes):
    _, cropped_size, _ = sizes # [200, 88, 96]
    coord = generate_coord(points, refpoint, new_size, angle, trans, sizes) # 离散化 + 归一化到[-1, 1]的网格点
    print("Before Scattering: ", coord)
...
class V2VVoxelization(object):
    def __init__(self, cubic_size, augmentation=True):
        ...
        output_size = int(self.cropped_size / self.pool_factor) # 44

        # Note, range(size) and indexing = 'ij'
        self.d3outputs = np.meshgrid(
            np.arange(output_size), np.arange(output_size), np.arange(output_size),
            indexing='ij'
        )
    ...
    def __call__(self, sample):
        ...
        '''
        points -> [76800, 3] refpoint -> [3]
        new_size: 100 angle: 0 trans: 4.0 self.sizes: [200, 88, 96] 
        '''
        normalize_coord, input = generate_cubic_input(points, refpoint, new_size, angle, trans, self.sizes)
        heatmap = generate_heatmap_gt(keypoints, refpoint, new_size, angle, trans, self.sizes, self.d3outputs,
                                      self.pool_factor, self.std)

        return input.reshape((1, *input.shape)), heatmap, normalize_coord
