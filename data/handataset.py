...
"""
from data.stb import STBDataset
from data.rhd import RHDDataset, RHDDataset_train
...
class HandDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.joint_root_idx = joint_root_idx  # root
    ...
    def __getitem__(self, index):
        ...
        # ''' jointRS.shape (21, 3) to location map (21, 3, 64, 64)'''
        ...
        if valid_dep and dep is not None:
            ...
            ''' to float array '''
            dep, dep_256_ori = ds.norm_dep_img(dep, joint_z=joint[:, 2])
            dep_256 = dep
        ...
        mask_256 = dep_256.copy()
        np.putmask(mask_256, mask_256 > 1e-2, 1.0)
        np.putmask(mask_256, mask_256 <= 1e-2, 0.0)
        ...
        # ''' Generate Visible map and Occlusion Map '''
        # max_value = 1.2
        # hm_sum = np.sum(hm, 0)
        # Visible_Map = hm_sum.copy()
        # np.putmask(Visible_Map, Visible_Map > max_value, 2.0)
        # np.putmask(Visible_Map, Visible_Map <= max_value, 0.0)
        # np.putmask(Visible_Map, Visible_Map > 1e-1, 1.0)
        #
        # Occlusion_Map = hm_sum.copy()
        # np.putmask(Occlusion_Map, Occlusion_Map > 1e-1, 1.0)
        ...
        dep_256 = torch.from_numpy(dep_256).float()
        mask_256 = torch.from_numpy(mask_256).float()
        dep_256_ori = torch.from_numpy(dep_256_ori).float()
        ...
        cam_ortho = torch.from_numpy(cam_param).float()
...
class HandDataset_(torch.utils.data.Dataset):
    def __init__(
            self,
            data_split='train',
            data_root="/disk1/data",
        ...
        super(HandDataset_, self).__init__()
